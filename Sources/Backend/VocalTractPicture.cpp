...
VocalTractPicture::VocalTractPicture(VocalTract *vocalTract)
{
  ...
  // Poster data, poster is an opengl object.
...
void VocalTractPicture::setVocalTract(VocalTract *vocalTract)
{
  this->tract = vocalTract;
}
...
{
  int width = this->picSize.x;
  int height = this->picSize.y;
...
void VocalTractPicture::GetSize(int &pictureWidth, int &pictureHeight)
{
  pictureWidth = this->picSize.x;
  pictureHeight = this->picSize.y;
  std::cout << pictureWidth << "\t" << pictureHeight << endl;
}

void VocalTractPicture::drawTest()
{
...
//    glClearColor(0.9f, 0.9f, 0.9f, 0);      // Light-gray background
  ...
  // ****************************************************************
  // Get 2D display area. Fixed value for each parameters.
  // ****************************************************************
...
void VocalTractPicture::get2DRegion(double &left_cm, double &right_cm, double &bottom_cm, double &top_cm)
{
  ...
  int w = this->picSize.x;
  int h = this->picSize.y;
  if (w < 1) { w = 1; }
...
// bool VocalTractPicture::saveImageBmp(const char &fileName)
// {
