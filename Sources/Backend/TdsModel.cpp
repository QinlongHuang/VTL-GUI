...
// double max_fullAmp = 0.0;
// double max_targetkHzAmp = 0.0;
// double tmp_gain = 0.0;
// double tmp_area = 0.0;
// double tmp_flow = 0.0;
  ...
  double& skinFlow_cm3_s, const string &matrixFileName)
{
  ...
  // cout << "---------------------------------------------------------------------------------------------------------------------------------------------------------" << endl;
  // cout << "Sample #" << getSampleIndex() << endl;
  ...
  // cout << "mouthFlow_cm3_s(64->93, 94)=" << mouthFlow_cm3_s << "  nostrilFlow_cm3_s(83->95, 96)=" << nostrilFlow_cm3_s 
  // << "  skinFlow_cm3_s(25->26)=" << skinFlow_cm3_s << "  radiatedFlow=" << radiatedFlow 
  // << "  max_velocity=" << max_velocity  << "  max_fullAmp=" << max_fullAmp << "  max_targetkHzAmp=" << max_targetkHzAmp 
  // << "  tmp_gain=" << tmp_gain << "  tmp_area=" << tmp_area << "  tmp_flow=" << tmp_flow
  // << "  max_R=" << max_R << "  max_D=" << max_D 
...
void TdsModel::prepareTimeStep()
{
  ...
  for (i=0; i < Tube::NUM_SECTIONS; i++)
  {
    ...
    ts->S = 0.0;      // No pressure source at the inlet of the section，section入口处无压力
  ...
  for (i=0; i < Tube::NUM_SECTIONS; i++)
  {
    ...
    /* R可能受u影响（即受magnitude影响），会变得很大。
     * E比较正常
     * D会很大，主要是受ts->pressure影响，而ts->pressure又受上一次的D和magnitude影响。
     * D会影响solutionVector的初值
     */
    // cout << "  Tube Section #" << i << ":"
    //   << "  pos=" << ts->pos << "  volume=" << ts->volume << "  area=" << ts->area << "  length=" << ts->length
    //   << "  ts->R[0]=" << ts->R[0] << "  ts->R[1]=" << ts->R[1] << "  ts->E=" << ts->E << "  ts->D=" << ts->D 
    //   << "  ts->currentOut[0]=" << ts->currentOut[0] << "  ts->currentOut[1]=" << ts->currentOut[1]
    //   << endl;

    // if (fabs(ts->D) > max_D)  { max_D = fabs(ts->D); }
    // if (fabs(ts->R[0]) > max_R)  { max_R = fabs(ts->R[0]); }

    // if (ts->R[0] > 100000000) { getchar(); }
  }
}
...
void TdsModel::calcNoiseSources()
{
  ...
  for (k=0; k < numConstrictions; k++)
  {
    ...
    // if (fabs(cons->velocity) >max_velocity) { max_velocity = fabs(cons->velocity); }
    ...
    if (cons->obstacleSection != -1)
    {
      ...
      // if (fabs(upstreamSource->targetAmp1kHz) > max_targetkHzAmp) 
      // { 
      //   max_targetkHzAmp = fabs(upstreamSource->targetAmp1kHz); 
      // }
      // if (fabs(downstreamSource->targetAmp1kHz) > max_targetkHzAmp) 
      // { 
      //   max_targetkHzAmp = fabs(downstreamSource->targetAmp1kHz); 
      // }

      // cout << "In NoiseSource[" << k << "]:  upstreamSource->target=" << upstreamSource->targetAmp1kHz << "  cons->fullAmp=" << cons->fullAmp
      //   << "  cons->gain=" << cons->gain << "  cons->velocity=" << cons->velocity << "  cons->area=" << cons->area << "  cons->flow=" << cons->flow
      //   << endl;
    }
  }
...
void TdsModel::calcNoiseSample(NoiseSource *s, double ampThreshold, bool printFlag)
{
  double delta = 0.0;
...
void TdsModel::calcMatrix()
{
  ...
  // ****************************************************************
  // 每一条branch current，都有一个source ts和target ts。
  // bc数量可以多于ts数量，比如咽腔末端的ts可以流到口腔，也可以流到鼻腔，这就是不同的bc。体现在ts->currentOut[0] or [1]上
  // 通常，bc的每一个sourceSection都是上一段管（i-1），targetSection是当前管（i）
  // 鼻腔的sourceSection是咽腔的最后一个section
  // bc在嘴部和鼻腔的最后一个section后分别添加了两个branch current，且都没有target section
  // 
  // ****************************************************************
  
  // cout << "In calcMatrix:" << endl;
  ...
  for (i=0; i < NUM_BRANCH_CURRENTS; i++)
  {
    ...
    // **************************************************************
    {
      ...
      if (branchingOffCurrent != -1)
      {
        ...
        // uB和uD就是出现了branching off，即sourceTs会有两个currentOut
        // u本质上就是bc.mag，用来更新H，而H和D用于更新solutionVector
        // cout << "  Branch Current #" << i << ":"
        //   << "  branchingOffCurrent=" << branchingOffCurrent 
        //   << "  uB=" << uB << "  uD=" << uD <<"  F=" << F << "  G=" << G << "  H=" << H << "  S=" << S
        //   << "  matrix[i][i]=" << matrix[i][i]
        //   << "  solutionVector=" << solutionVector[i]
        //   << endl;
      }
      else
      // ************************************************************
    ...
    // if (fabs(F) > max_F) { max_F = fabs(F); }
    // if (fabs(G) > max_G) { max_G = fabs(G); }
    // if (fabs(H) > max_H) { max_H = fabs(H); }
...
void TdsModel::updateVariables()
{
  ...
  for (i=0; i < NUM_BRANCH_CURRENTS; i++)
  {
    ...
    //   << "  ts_old.mag=" << oldCurrent << "  ts_new.mag=" << bc->magnitude
    //   << endl;
  }
  ...
  for (i=0; i < Tube::NUM_SECTIONS; i++)
  {
    ...
    // cout << "  Tube Section #" << i << ":"
    //   << "  ts_old->pressure=" << oldPressure << "  ts_new->pressure=" << ts->pressure << "  netFlow=" << netFlow
    //   << endl;
...
void TdsModel::solveEquationsCholesky()
{
  ...
  // cout << "In solveEquationsCholesky:  " << endl;
  for (k = NUM_BRANCH_CURRENTS - 1; k >= 0; --k)
  {
    ...
    // cout << "  Branch Current #" << k << ":"
    //   << "  flowVector=" << flowVector[k] << "  solutionVector=" << solutionVector[k] << "  factorizationMatrix=" << factorizationMatrix[k][k]
    //   << endl;
    // if (fabs(solutionVector[k]) > max_solutionV) { max_solutionV = fabs(solutionVector[k]); }
  }
}
