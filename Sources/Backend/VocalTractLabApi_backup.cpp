...
#include "VocalTractLabApi_backup.h"
#include "Dsp.h"
#include "SoundLib.h"
#include "AudioFile.h"
...
static AnatomyParams *anatomyParams = NULL;
static VocalTractPicture *vtPicture = NULL;
...
int vtlSaveVocalTractVideoFrame(double duration_s, double *tractParams, const char *folderName)
{
  if (!vtlApiInitialized)
  {
    printf("Error: The API has not been initialized.\n");
  ...
  for (int frameIndex = 0; frameIndex < numFrames; frameIndex++)
  {
    ...
    newVocalTractParams = &tractParams[frameIndex*VocalTract::NUM_PARAMS];
    for (i=0; i < VocalTract::NUM_PARAMS; i++)
    {
      vocalTract->param[i].x = newVocalTractParams[i];
    }
    vocalTract->calculateAll();
...
int vtlSetAnatomyParams(double *params)
{
  if (!vtlApiInitialized)
  {
    printf("Error: The API has not been initialized.\n");
...
int vtlGetAnatomyParams(double *ana_params)
{
  if (!vtlApiInitialized)
  {
    printf("Error: The API has not been initialized.\n");
  ...
  for (int i=0; i<AnatomyParams::NUM_ANATOMY_PARAMS; i++)
  {
    double param = anatomyParams->param[i].x;
    ana_params[i] = param;
  }
  ...
  int frameStep_samples)
{
  ...
  if (!vtlApiInitialized)
  {
    throw runtime_error("Error: The API has not been initialized.");
    return audio;
  }
  ...
  for (i = 0; i < numFrames; i++)
  {
    if (i == 0)
    {
      ...
      vector<double> audioVector;
      synthesizer->add(&glottisParams[i*VocalTract::NUM_PARAMS], 
        &tractParams[i*numGlottisParams], 
        0, audioVector);
    }
    else
    {
  ...
  int numFrames, int frameStep_samples, double *audio, int enableConsoleOutput)
{
  ...
  for (i = 0; i < numFrames; i++)
  {
    ...
    if ((enableConsoleOutput != 0) && ((i % 20) == 0))  // print a dot every 20 frames
    {
      printf(".");
    }
  }
